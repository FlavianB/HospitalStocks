...
public class PDFService {
    ...
    public String generateConsumptionPDF(Consumption consumption) {
        String pdfPath = "consumption_" + consumption.getId() + ".pdf";
        Document document = new Document();
        ...
        try {
            ...
            // Adding title with styling
            Font titleFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 20, BaseColor.BLACK);
            Paragraph title = new Paragraph("Consumption Details", titleFont);
            title.setAlignment(Element.ALIGN_CENTER);
            title.setSpacingAfter(20);
            document.add(title);

            // Adding table with consumption details
            PdfPTable table = new PdfPTable(2);
            table.setWidthPercentage(100);
            table.setSpacingBefore(10f);
            table.setSpacingAfter(10f);
            ...
            // Adding consumption details to the table
            addTableRow(table, "Consumption ID", consumption.getId().toString());
            addTableRow(table, "Date", consumption.getDate().toString());
            addTableRow(table, "Quantity", String.valueOf(consumption.getQuantity()));
            addTableRow(table, "Drug", consumption.getDrug().getName());
